...
**2026-04-30 — Pass 3 Faz-A partial apply (2 knob).** Optuna 3-seed walk-forward 73/27 tune (12 düğme: per-pillar weights 24 + confluence_threshold + 3 hard-gate toggle + target_rr_ratio + zone_max_wait_bars), mean obj +163.1, stdev 3.0 — istikrarlı tune. OOS gate borderline fail (validate net_R 0.45× train, WR drop 5.7pp), full apply overfit riski. **Operator partial apply karar: sadece 2 düğme** (counter-factual ile bağımsız teyitli):  `analysis.cross_asset_veto_enabled` true→false (Pass 2.5.F counter-factual WR 51.4% n=56 — bot 50/50 setupları reddediyordu); `execution.zone_max_wait_bars` 2→4 (counter-factual WR 45.7% n=606 + Optuna top-10 trial'da hepsi 4 önerdi; pending 6→12 dk wait). 22 knob (per-pillar weights 24, target_rr_ratio 1.5, confluence_threshold 3.75, vs.) **SKIP** — daha çok demo data toplandıktan sonra Faz-B. Pre-warm KlineCache 1633 row, replay engine 3 yeni knob (pillar_weights aktif, target_rr_ratio recompute+rewalk, zone_max_wait_bars gate-toggle). Test suite 55/55 ✓.

**2026-05-01 — VWAP daily-reset blackout 10dk → 60dk.** DB'de Bybit-era UTC 00:00-02:00 penceresi 19 trade WR 26.3% / -10.51R / -$139 (UTC 00h: 8 trade WR 25% -5.30R; UTC 01h: 8 trade WR 25% -4.09R). Daily session-anchored VWAP soft-pillar 10dk cutoff sonrasında bile band-stabilisation kuyruğunda near-noise okuyup spurious confluence ağırlığı üretiyordu. `analysis.vwap_reset_blackout_window_post_min: 10 → 60` (pre_min 0 sabit). UTC 00:00-01:00 yeni entry + resting pending iptal — kod değişmedi (mekanizma `runner.py:1972` + `evaluate_pending_invalidation_gates` zaten yerinde), tek YAML knob bump. Validator `[0, 60]` cap'i sınır değer; daha geniş window istenirse `config.py:299-317` cap'i de büyütülmeli.

**2026-05-02 — Phase A.9 ADX numeric capture (entry TF + HTF).** Journal `trades` + `rejected_signals` gain six new nullable columns each: `adx_3m_at_entry`, `plus_di_3m_at_entry`, `minus_di_3m_at_entry`, `adx_15m_at_entry`, `plus_di_15m_at_entry`, `minus_di_15m_at_entry`. `trend_regime_at_entry` label-only column persists; raw values let Pass 3 GBT learn its own ADX cutoff (instead of our ad-hoc 20/30) and adds direction-strength signal via `+DI`/`-DI`. Multi-TF (3m + 15m) capture enables divergence features ("3m strong but 15m flat"). Zero extra TV/Bybit calls — entry-TF triad already computed for `_run_one_symbol`'s regime label; HTF triad reuses the htf_candles buffer fetched for S/R + state in the HTF pass (new `BotContext.htf_adx_cache`, popped on already-open / refresh error symmetric with `htf_state_cache`). Placement-time stash on `PendingSetupMeta` so pending-fill journal rows stamp the regime that drove the limit (not the fill moment). Helper `_adx_triad_kwargs(prefix, result)` writes NULL for UNKNOWN/None to keep "insufficient data" distinguishable from "computed zero" downstream. 10 new tests; full suite 1223/1223 ✓.

**2026-05-02 — Phase A.8 weakening-momentum defensive exit.** New gate `_maybe_close_on_momentum_fade` ([runner.py](src/bot/runner.py)). Each cycle for OPEN positions: (1) compute `score_direction(state, position_direction)` for raw directional confluence; (2) append to `_Tracked.recent_confluence_history` (truncated to `weakening_max_history=8`); (3) if last `weakening_min_cycles=3` entries form a monotonic decline with each step ≥ `weakening_min_score_drop=0.5` AND `mfe_r_high ≥ weakening_min_mfe_r=0.5`, fire `_defensive_close()` with reason `momentum_fade` → journal stamps new close_reason `EARLY_CLOSE_MOMENTUM_FADE`. Operator: "aynı yönlü daha düşük veriler gelmeye devam ediyorsa cyclelarda hareketin yavaşladığını düşünüp pozisyonda kalıcılığı istemeyeceğiz". `_defensive_close()` close_reason now derived from `reason` arg via map (LTF reversal still works). Snapshot writer also populates the Phase A.7 `confluence_score_now` column from the latest history entry. 9 new tests; full suite 1213/1213 ✓.

**2026-05-02 — Phase A.7 snap cadence 300→180s + `confluence_score_now` column.** Operator-decided snapshot cadence default 300 → 180s (= ~1 cycle on 3m TF). All snapshot data comes from cached state; no extra TV/Bybit calls regardless of cadence. New nullable column `position_snapshots.confluence_score_now` (signed: positive aligned, negative opposing) — stub only this commit; populated + consumed in Phase A.8 weakening-momentum exit. Idempotent ALTER for existing DBs. PositionSnapshotRecord pydantic model + writer + reader updated. 3 round-trip tests; full suite 1204/1204 ✓.

**2026-05-02 — Phase A.6 MAE-triggered BE-lock with LIMIT-based exit.** New gate `_maybe_lock_sl_on_mae_recovery` ([runner.py](src/bot/runner.py)). Two-stage: (1) MAE crosses `mae_be_lock_threshold_r=-0.6R` → arm; (2) mark recovers within `mae_be_lock_recovery_band_r=0.1R` of entry AND LTF trend opposes position → place reduce-only post-only LIMIT at `entry × (1 ± sl_be_offset_pct)`. LONG sells just above entry (micro-profit on touch from below); SHORT buys just below. Maker exit, fee-positive close. Position-attached SL at -1R stays as backup. One-shot via `_Tracked.mae_be_lock_applied` flag. Same-cycle stage-1+2 forbidden (one-cycle delay enforces a deliberate "wait for confirmation" beat). Close-path orphan cleanup: monitor cancels the BE-recovery limit if the position closed via SL/TP first. 12 new tests; full suite 1201/1201 ✓.

**2026-05-02 — Phase A.5 multi-step trailing SL after MFE-lock.** New gate `_maybe_trail_sl_after_mfe` ([runner.py:1495](src/bot/runner.py:1495)) pulls SL forward in `trail_step_r` (0.5R) chunks once MFE crosses `trail_arm_at_mfe_r` (1.5R). Wick-safe: `trail_distance_r=0.5R` behind MFE clears typical 3m wicks. Monotonic-only via `_Tracked.last_trail_lock_r` + new `monitor.trail_sl_to()`. Disabled in RANGING (TP@1.2R fires before arm). Math: MFE 1.5→SL+1R, MFE 2→SL+1.5R, MFE 2.5→SL+2R. Distinct from one-shot MFE-lock (still active at 1R→BE+fee). 10 new tests; full suite 1189/1189 ✓.

**2026-05-02 — Phase A.4b RANGING-only VWAP-band SL anchor.** `build_zone_setup()` ([setup_planner.py:342](src/strategy/setup_planner.py:342)) gains `trend_regime` kwarg. RANGING + LONG → SL=`max(zone_sl, vwap_3m_lower - sl_buffer×ATR)`; RANGING + SHORT → SL=`min(zone_sl, vwap_3m_upper + sl_buffer×ATR)`. `max`/`min` picks tighter candidate; band-anchored SL only wins when CLOSER to entry. Per-symbol `min_sl_distance_pct` floor still applies. Other regimes / UNKNOWN / None → no override. 10 new tests.

**2026-05-02 — Phase A max-profit doctrine (commits 1-3 + 4 YAML).** Operator doktrin reaffirm: WR yerine **max realized R**. Phase A Commits 1-3 (no behavior): regime-aware `target_rr_ratio_per_regime` + `tp_min_rr_floor_per_regime` config schema, `_Tracked.regime_at_entry` thread, `_maybe_revise_tp_dynamic` per-regime lookup. **Commit 4 — first behavior change**: YAML apply `target_rr_ratio_per_regime: {RANGING: 1.2, WEAK_TREND: 1.5, STRONG_TREND: 2.5}` + lockstep `tp_min_rr_floor_per_regime: {0.6, 0.7, 1.2}` (each ~50% of target). Mantık: RANGING TP yakın (give-back azaltır), WEAK aynı, STRONG let-it-run (Commit 5 trailing safety net'i ile). UNKNOWN / pre-Phase-A rehydrate → global 1.5R. Counterfactual'da +3R/71-trade beklenti (snapshot-based conservative bound; gerçek kline-walk daha yüksek olabilir).

**2026-05-01 — Pass 3 Faz-A revert + zone_wait tighten (3 dk) + cross_asset eth_anchored mode.** Faz-A apply'dan sonra 19 trade post-window WR 36.8% / -5.02R (PF 0.65) vs PRE son-19 WR 63.2% / +5.85R; ~10R'lik delta tek atomik apply'dan. Knob bazlı kırılım: yeni window'da fill edilen 5 trade (bars>2) 1W/4L (-3.44R), bars=2 dilimi de hasarlı (1W/4L, -3.35R) — window genişletme hipotezi canlı veride çürüdü. Altcoin (ETH+SOL+XRP) post-apply 1W/6L; cross_asset gate kapalıyken altcoin koruması yok. **Adım 1 — atomik YAML revert + tighten** (commit `7cf99c8`): `cross_asset_veto_enabled` false→true; `zone_max_wait_bars` 4→1 (~3 dk; PRE bars=1 sweet spot WR 66.7%/+8.45R'a odaklan, per-cycle yeniden değerlendirme). **Adım 2 — cross_asset implementation tighten** (operator pair-bound model): yeni YAML knob `cross_asset_veto_mode: "eth_anchored"` (default `"both"` = legacy back-compat). `eth_anchored` mantığı: BTC entry → ETH ters → reddet (yeni gate); ETH entry → BTC ters → reddet (yeni gate); altcoin entry → BTC OR ETH ters → reddet (mevcut AND'den daha sıkı); pillar bias missing/neutral/stale → fail-CLOSED (her yön bloklanır, sentinel `Direction.UNDEFINED`). Operator: "btc ve eth direction'a kesin bakılmalı". Revert kolay: YAML'da `"both"` yazıp restart yeter (kod değişmez). Test 1146/1146 ✓ (9 yeni eth_anchored test eklendi).
...
`displacement_candle` · `ema_momentum_contra` · `vwap_misaligned` · `cross_asset_opposition` (mode-aware: `eth_anchored` default = BTC↔ETH simetrik pair-bound + altcoin BTC OR ETH ters + fail-CLOSED on missing/stale; `both` legacy = altcoin only, BTC AND ETH ters, fail-open) · `vwap_reset_blackout` (UTC 00:00-01:00 daily VWAP re-anchor window). *`premium_discount_zone` + `htf_tp_ceiling` wired but disabled (Pass 3 soft-weighted re-add candidates). Whale `whale_transfer_blackout` gate REMOVED 2026-04-22; directional intuition moved to `flow_alignment` soft signal.*
