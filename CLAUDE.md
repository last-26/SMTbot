...
**2026-05-01 — Pass 3 Faz-A revert + zone_wait tighten (3 dk hipotezi).** Faz-A apply'dan sonra 19 trade post-window WR 36.8% / -5.02R (PF 0.65) vs PRE son-19 WR 63.2% / +5.85R; ~10R'lik delta tek atomik apply'dan. Knob bazlı kırılım: yeni window'da fill edilen 5 trade (bars>2) 1W/4L (-3.44R), bars=2 dilimi de hasarlı (1W/4L, -3.35R) — window genişletme hipotezi canlı veride çürüdü. Altcoin (ETH+SOL+XRP) post-apply 1W/6L; cross_asset gate kapalıyken altcoin koruması yok. **Atomik revert + tighten:** `cross_asset_veto_enabled` false→true (mevcut "BTC AND ETH ters" mantığı baseline), `zone_max_wait_bars` 4→1 (~3 dk; PRE bars=1 sweet spot WR 66.7%/+8.45R'a odaklan, per-cycle yeniden değerlendirme). Cross_asset implementation tighten (ETH-anchored / 2c — BTC↔ETH simetrik + altcoin BTC OR ETH ters → reddet) ayrı PR'a ertelendi.
